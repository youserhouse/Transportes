...
| `state.js` | Global `state` object `{ prov, zona, country, cpPrt }`, country-switching UI, Portugal CP resolution logic. Also owns `appSettings` (`{ requireCp, requireCliente, dashHiddenCols }`), persisted to `localStorage` (`transportes_settings`) — see Configuraciones below. |
...
- **`appSettings`** (`state.js`): `{ requireCp: true, requireCliente: true, dashHiddenCols: [] }`, persisted to `localStorage` (`transportes_settings`), loaded immediately on script eval and synced to the Configuraciones toggle UI on `DOMContentLoaded`. `requireCp`/`requireCliente` gate validation in both `calcular.js` (Palés) and `cajas.js` (Cajas). `dashHiddenCols` is an array of column keys (from `DASH_COL_KEYS` in `state.js`, mirroring `DASH_HIST_COLUMNS` in `dashboard.js`) hidden from the Dashboard's "Historial de cálculos" table — toggled per-column in Configuraciones → "Columnas del historial (Dashboard)" via `toggleDashColumn(key)`. CSV export (`exportarHistorialDashboardCSV`) always includes every column regardless of this setting.
