...
The legacy standalone React dashboard (`Dashboard.html`) has been removed — it was no longer linked from `index.html`'s nav and no longer listed in `sw.js`'s `ASSETS`. The Dashboard experience lives entirely inside `index.html`'s `mode-dashboard` tab via `dashboard.js`.
...
```
theme.js → helpers.js → data.js → state.js → guardar.js → ui.js → calcular.js → cajas.js → dashboard.js
```

| File | Responsibility |
|------|---------------|
| `index.html` | App shell: Palés/Cajas/Configuraciones/Dashboard mode tabs, Firebase init + auth overlay, CSP meta tag, inline `window.fsGuardarCalculo` / `window.fsListenCalculos` / `window.fsActualizarCalculo` (Firestore CRUD for `calculos` collection), plus `window.fsListenConfiguracion` / `window.fsGuardarConfiguracion` (Firestore CRUD for the single shared `app_settings/global` document — global Configuraciones state, see `appSettings` below). |
| `data.js` | **Single source of truth for all tariff tables and geographic maps.** Edit only this file to update prices or zones. |
| `theme.js` | Light/dark theme toggle, persisted to `localStorage` (`transportes_theme`), shared by `index.html`, `login.html`, `splash.html`. |
| `helpers.js` | Small shared utilities used by `ui.js`, `calcular.js`, `cajas.js`: `formatProvincia(p)` (uppercase province name → display-cased, e.g. `"BARCELONA"` → `"Barcelona"`); `validarCliente(cliente)` / `validarCp(cp, digitos, pais)` (shared `appSettings.requireCliente`/`requireCp` gating, used by both `calcular()` and `calcularCajas()`, returning an error message string or `null`); `mostrarError(errEl, mensaje)` / `ocultarError(errEl)` (toggle the shared `.error-banner`/`.show` CSS classes used by both `#error-msg` and `#error-cajas-msg`). Add new cross-mode helpers here instead of duplicating logic across the Palés/Cajas files. |
| `state.js` | Global `state` object `{ prov, zona, country, cpPrt }`, country-switching UI, Portugal CP resolution logic. Also owns `appSettings` (`{ requireCp, requireCliente, dashHiddenCols }`), synced live from the shared Firestore document `app_settings/global` (not `localStorage` — see Configuraciones below). |
...
| `dashboard.js` | Native embedded Dashboard (KPIs, comparativa, evolución, distribución, provincias, historial table) rendered inside `index.html`'s `mode-dashboard` tab. Reads **live** from the Firestore `calculos` collection via `window.fsListenCalculos` (`onSnapshot`) — no `localStorage` involved. The historial table columns (including "Envío", the `elegido` carrier per record, and "Portes", the cost of that chosen carrier via `dashPrecioElegido(c)`) are driven by the `DASH_HIST_COLUMNS` config array, filtered against `appSettings.dashHiddenCols` (see Configuraciones below) — the grid's column widths are set at render time via the `--dash-cols` CSS custom property on `#dash-table-inner`, so hidden columns don't leave blank space. |
...
**Toggling surcharges on/off:** `calcPalletways()`/`calcPallettaysManual()` (`ui.js`) and `calcCeva()`/`calcCevaByKg()`/`calcCevaPrt()` (`ui.js`/`cajas.js`/`data.js`) each take an optional trailing boolean parameter (`aplicarPorte`/`aplicarRecargo`, default `true`) that zeroes out the surcharge when `false`. Callers pass `appSettings.pwPorteActivo`/`appSettings.cevaRecargoActivo` explicitly — the pricing functions themselves never read `appSettings` directly, so they stay pure and testable without `state.js` in the test harness. The breakdown UI (`renderPW`/`renderCEVA` in `ui.js`, the Cajas breakdown in `cajas.js`) only renders the "Porte adicional"/"Recargo" line when the resulting surcharge is `> 0`, so a disabled surcharge simply disappears from the breakdown instead of showing "+0,00 €".
...
- **Postal code is required** before calculating, by default: 5 digits for Spain, 7 digits for Portugal. This requirement (along with requiring the client name) can be toggled in Configuraciones → "Validaciones obligatorias" — see `appSettings` below. Toggling `requireCp` off does **not** remove the underlying need to resolve a province/zona (Spain) or some CP-derived tariff lookup (Portugal); it only disables the strict full-digit-length check.
- **`appSettings`** (`state.js`): `{ requireCp: true, requireCliente: true, dashHiddenCols: [], pwPorteActivo: true, cevaRecargoActivo: true }`. **Global, not per-device**: synced live via Firestore from the single shared document `app_settings/global` (`window.fsListenConfiguracion` `onSnapshot`, inline module script in `index.html`) — any authenticated user changing a toggle in Configuraciones updates it for every other logged-in user/device in real time. `appSettings` starts out with hardcoded defaults (`APP_SETTINGS_DEFAULTS` in `state.js`) until the first snapshot arrives (the listener is registered on `DOMContentLoaded`, since the module script that defines `window.fsListenConfiguracion` is deferred and guaranteed to run before that event); writes go through `window.fsGuardarConfiguracion(appSettings)` (`setDoc` with `{ merge: true }`), fired from `saveAppSettings()`. `requireCp`/`requireCliente` gate validation in both `calcular.js` (Palés) and `cajas.js` (Cajas). `dashHiddenCols` is an array of column keys (from `DASH_COL_KEYS` in `state.js`, mirroring `DASH_HIST_COLUMNS` in `dashboard.js`) hidden from the Dashboard's "Historial de cálculos" table — toggled per-column in Configuraciones → "Columnas del historial (Dashboard)" via `toggleDashColumn(key)`. CSV export (`exportarHistorialDashboardCSV`) always includes every column regardless of this setting. `pwPorteActivo`/`cevaRecargoActivo` toggle the +4.7%/+10.7% surcharges (Configuraciones → "Recargos") — see "Toggling surcharges on/off" in Pricing Logic above.
...
- **Firebase project**: `transporte-99482` (configured in `index.html`).
...
- **Firestore security rules**: `firestore.rules` (root, mirrored in `firebase.json`/`.firebaserc`) is versioned here to match what's deployed in Firebase Console → Firestore Database → Reglas. It enforces server-side that only signed-in users present in `authorized_users` can read/write `calculos` and `app_settings` — the client-side check in `index.html`/`login.html` alone would not be sufficient, since any Google account can authenticate via Firebase Auth regardless of `authorized_users` membership. **If you edit rules in the console, update this file too** (or deploy this file via `firebase deploy --only firestore:rules`) so they don't drift apart. **The `app_settings` rule is currently only in the repo and still needs a manual `firebase deploy --only firestore:rules` (or console paste) to take effect in production** — see the header comment in `firestore.rules`.
...
- Configuraciones settings (`appSettings`) follow the same live-sync pattern but against a single document instead of a collection: `window.fsListenConfiguracion` (`onSnapshot(doc(db, 'app_settings', 'global'), ...)`) and `window.fsGuardarConfiguracion(datos)` (`setDoc(..., { merge: true })`) — see `appSettings` in Important Conventions above.
