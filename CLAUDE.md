...
- **Caveat**: the "✓ GUARDADO" UI feedback in `guardar.js` fires as soon as the local write resolves — with offline persistence this happens even while offline (the write is only queued locally, not yet confirmed by the server). There's no `navigator.onLine` check or "pendiente de sincronizar" indicator, so a save made offline currently looks identical to one made online.
